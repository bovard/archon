...
DataCollector.prototype = {
    ...
    printHostSummary: function(host, teams) {
        var i;
        var winLoss = {};
        for (i = 0; i < teams.length; i++) {
            winLoss[teams[i]] = [0,0];
        ...
        console.log('matchup', '\t', 'wins', '\t', 'losses', '\t', '%')
        for (var team in winLoss) {
            var per = Math.floor(winLoss[team][WIN] / (winLoss[team][LOSS] + winLoss[team][WIN])*100);
            console.log(host + ' vs ' + team, '\t', winLoss[team][WIN], '\t', winLoss[team][LOSS], '\t', per);
        }
    },
    ...
    printPlayerMapSummary: function(team, maps, series) {
        _printPlayerMapSummary(team, maps, this._getGames(series));
    },
    reset: function() {
        this._entries = [];
    },
...
function _printPlayerMapSummary(team, maps, games) {
    ...
    console.log('');
    console.log('Summary:');
    console.log('map_name', '\t', 'wins', '\t', 'losses', '\t', '%')
