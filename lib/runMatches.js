...
function runMatches(maps, teams, host, cores, series) {
    ...
    function startNextGame() {
        ...
        if (series) {
            ...
            runGame(game[MAP], game[TEAM_A], game[TEAM_B], seriesData, endGame);
        } else {
            runGame(game[MAP], game[TEAM_A], game[TEAM_B], series, endGame);
        }
    }

    // start as many games running as we set cores or gameLength
    for (var i = 0; i < Math.min(cores, gamesLength); i++) {
        startNextGame();
    }
}
