...
function writeConf(map, teamA, teamB, replayFolder) {
    ...
    confFileContents += '\n' + TEAM_B_CONF + teamB;
    if (map.indexOf(',') !== -1) {
        confFileContents += '\n' + SAVE_FILE + path.join(replayFolder, ts + '_' + teamA + '_' + teamB + '.rms');
    } else {
        confFileContents += '\n' + SAVE_FILE + path.join(replayFolder, ts + '_' + teamA + '_' + teamB + '_' + map.slice(0, -4) + '.rms');
    }
