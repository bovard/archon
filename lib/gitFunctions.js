...
function _copyFiles(folder, newFolder, callback) {
    ...
    function temp(status) {
        if (files.length > 0 && status) {
            _copyFile(path.join(folder, files.shift()), newFolder, temp);
        } else {
            callback(status);
        }
    }
...
function _copyFile(file, newFolder, callback) {
    var cp = spawn('cp', ['-R', file, newFolder]);
...
function gitTags(callback) {
    var gitPath = _findPathToGitRepo();
    var gitStatus = spawn('git', ['tag'], {cwd: gitPath});
    var result = '';
